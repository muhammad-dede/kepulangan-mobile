...
class EditBastUdaraController extends GetxController {
  ...
  void getEditData() {
    try {
      purchaseOrderController.text = bastUdara?.purchaseOrder ?? "";
      penyediaJasaController.text =
          bastUdara?.penyediaJasa?.namaPerusahaan ?? "";
      idPenyediaJasa = bastUdara?.penyediaJasa?.id;
      alamatController.text = bastUdara?.alamat?.judul ?? "";
      idAlamat = bastUdara?.alamat?.id;
      durasiPengerjaanController.text = bastUdara?.durasiPengerjaan != null
          ? "${bastUdara?.durasiPengerjaan.toString()} hari pengerjaan"
          : "";
      durasiPengerjaan = bastUdara?.durasiPengerjaan;
      tanggalSerahTerima = bastUdara?.tanggalSerahTerima != null
          ? DateTime.parse(bastUdara!.tanggalSerahTerima.toString())
          : null;
      tanggalSerahTerimaController.text = bastUdara?.tanggalSerahTerima != null
          ? DateFormat('dd-MM-yyyy')
              .format(DateTime.parse(bastUdara!.tanggalSerahTerima.toString()))
          : "";
      fotoPenyediaJasaOld = bastUdara?.fotoPenyediaJasa;
      fotoSerahTerimaOld = bastUdara?.fotoSerahTerima;
      for (var udara in bastUdara!.udara!) {
        listUdara!.add(udara);
      }
    } catch (e) {
  ...
  Future<void> getImigran() async {
    try {
      ...
      final response = await BaseClient().get(
          "/api/imigran?id_kepulangan=2&id_bast_udara=${bastUdara?.id}&terlaksana=false");
      response.fold((l) {
        listAllImigran = [];
        listImigran = [];
      }, (r) {
    ...
    if (formState.currentState!.validate()) {
      try {
        ...
        final response = await BaseClient()
            .postMultipart("/api/bast-udara/update/${bastUdara?.id}", data);
        response.fold((l) {
