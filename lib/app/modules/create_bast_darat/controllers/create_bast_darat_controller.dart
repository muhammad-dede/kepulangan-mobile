...
class CreateBastDaratController extends GetxController {
  ...
  Future<void> getImigran() async {
    try {
      ...
      final response = await BaseClient()
          .get("/api/imigran?id_kepulangan=1&terlaksana=false");
      response.fold((l) {
        listAllImigran = [];
        listImigran = [];
      }, (r) {
  ...
  Future<void> save() async {
    if (formState.currentState!.validate()) {
      try {
        ...
        final response =
            await BaseClient().postMultipart("/api/bast-darat/store", data);
        response.fold((l) {
